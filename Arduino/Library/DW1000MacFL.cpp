...
//total=12 bytes
void DW1000MacFL::generateBlinkFrame(byte frame[], byte sourceAddress[], byte sourceShortAddress[]) {
	//Frame Control
	*frame     = FC_1_BLINK;
	//sequence number
	*(frame+1) = _seqNumber;
...
//total=9 bytes
void DW1000MacFL::generateShortMACFrame(byte frame[], byte sourceShortAddress[], byte destinationShortAddress[]) {
	//Frame controle
	*frame     = FC_1;
	*(frame+1) = FC_2_SHORT;
...
//total=15
void DW1000MacFL::generateLongMACFrame(byte frame[], byte sourceShortAddress[], byte destinationAddress[]) {
	//Frame controle
	*frame     = FC_1;
	*(frame+1) = FC_2;
	//sequence number
...
void DW1000MacFL::decodeBlinkFrame(byte frame[], byte address[], byte shortAddress[]) {
...
void DW1000MacFL::decodeShortMACFrame(byte frame[], byte address[]) {
...
void DW1000MacFL::decodeLongMACFrame(byte frame[], byte address[]) {
	byte reverseAddress[2];
...
void DW1000MacFL::incrementSeqNumber() {
...
void DW1000MacFL::reverseArray(byte to[], byte from[], int16_t size) {
