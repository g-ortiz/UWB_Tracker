 ...
 * Decawave DW1000RL library for arduino.
 *
 ...
 *
 * @file DW1000RL.cpp
 * Arduino driver library (source file) for the Decawave DW1000RL UWB transceiver IC.
 */

#include "DW1000RL.h"

DW1000RLClass DW1000RL;

/* ###########################################################################
 * #### Static member variables ##############################################
 * ######################################################################### */
// pins
uint8_t DW1000RLClass::_ss;
uint8_t DW1000RLClass::_rst;
uint8_t DW1000RLClass::_irq;


// IRQ callbacks
void (* DW1000RLClass::_handleSent)(void)                      = 0;
void (* DW1000RLClass::_handleError)(void)                     = 0;
void (* DW1000RLClass::_handleReceived)(void)                  = 0;
void (* DW1000RLClass::_handleReceiveFailed)(void)             = 0;
void (* DW1000RLClass::_handleReceiveTimeout)(void)            = 0;
void (* DW1000RLClass::_handleReceiveTimestampAvailable)(void) = 0;

// registers
byte       DW1000RLClass::_syscfg[LEN_SYS_CFG];
byte       DW1000RLClass::_sysctrl[LEN_SYS_CTRL];
byte       DW1000RLClass::_sysstatus[LEN_SYS_STATUS];
byte       DW1000RLClass::_txfctrl[LEN_TX_FCTRL];
byte       DW1000RLClass::_sysmask[LEN_SYS_MASK];
byte       DW1000RLClass::_chanctrl[LEN_CHAN_CTRL];
byte       DW1000RLClass::_networkAndAddress[LEN_PANADR];

// monitoring
byte DW1000RLClass::_vmeas3v3 = 0;
byte DW1000RLClass::_tmeas23C = 0;

// driver internal state
byte       DW1000RLClass::_extendedFrameLength = FRAME_LENGTH_NORMAL;
byte       DW1000RLClass::_pacSize             = PAC_SIZE_8;
byte       DW1000RLClass::_pulseFrequency      = TX_PULSE_FREQ_16MHZ;
byte       DW1000RLClass::_dataRate            = TRX_RATE_6800KBPS;
byte       DW1000RLClass::_preambleLength      = TX_PREAMBLE_LEN_128;
byte       DW1000RLClass::_preambleCode        = PREAMBLE_CODE_16MHZ_4;
byte       DW1000RLClass::_channel             = CHANNEL_5;
DW1000Time DW1000RLClass::_antennaDelay;
boolean    DW1000RLClass::_smartPower          = false;

boolean    DW1000RLClass::_frameCheck          = true;
boolean    DW1000RLClass::_permanentReceive    = false;
uint8_t    DW1000RLClass::_deviceMode          = IDLE_MODE; // TODO replace by enum
...
constexpr byte DW1000RLClass::MODE_LONGDATA_RANGE_LOWPOWER[];
constexpr byte DW1000RLClass::MODE_SHORTDATA_FAST_LOWPOWER[];
constexpr byte DW1000RLClass::MODE_LONGDATA_FAST_LOWPOWER[];
constexpr byte DW1000RLClass::MODE_SHORTDATA_FAST_ACCURACY[];
constexpr byte DW1000RLClass::MODE_LONGDATA_FAST_ACCURACY[];
constexpr byte DW1000RLClass::MODE_LONGDATA_RANGE_ACCURACY[];
/*
const byte DW1000RLClass::MODE_LONGDATA_RANGE_LOWPOWER[] = {TRX_RATE_110KBPS, TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_2048};
const byte DW1000RLClass::MODE_SHORTDATA_FAST_LOWPOWER[] = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_128};
const byte DW1000RLClass::MODE_LONGDATA_FAST_LOWPOWER[]  = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_1024};
const byte DW1000RLClass::MODE_SHORTDATA_FAST_ACCURACY[] = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_128};
const byte DW1000RLClass::MODE_LONGDATA_FAST_ACCURACY[]  = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_1024};
const byte DW1000RLClass::MODE_LONGDATA_RANGE_ACCURACY[] = {TRX_RATE_110KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_2048};
*/
// range bias tables (500 MHz in [mm] and 900 MHz in [2mm] - to fit into bytes)
constexpr byte DW1000RLClass::BIAS_500_16[];
constexpr byte DW1000RLClass::BIAS_500_64[];
constexpr byte DW1000RLClass::BIAS_900_16[];
constexpr byte DW1000RLClass::BIAS_900_64[];
/*
const byte DW1000RLClass::BIAS_500_16[] = {198, 187, 179, 163, 143, 127, 109, 84, 59, 31, 0, 36, 65, 84, 97, 106, 110, 112};
const byte DW1000RLClass::BIAS_500_64[] = {110, 105, 100, 93, 82, 69, 51, 27, 0, 21, 35, 42, 49, 62, 71, 76, 81, 86};
const byte DW1000RLClass::BIAS_900_16[] = {137, 122, 105, 88, 69, 47, 25, 0, 21, 48, 79, 105, 127, 147, 160, 169, 178, 197};
const byte DW1000RLClass::BIAS_900_64[] = {147, 133, 117, 99, 75, 50, 29, 0, 24, 45, 63, 76, 87, 98, 116, 122, 132, 142};
*/
// SPI settings
const SPISettings DW1000RLClass::_fastSPI = SPISettings(16000000L, MSBFIRST, SPI_MODE0);
const SPISettings DW1000RLClass::_slowSPI = SPISettings(2000000L, MSBFIRST, SPI_MODE0);
const SPISettings* DW1000RLClass::_currentSPI = &_fastSPI;

/* ###########################################################################
 * #### Init and end #######################################################
 * ######################################################################### */

void DW1000RLClass::end() {
	SPI.end();
}

void DW1000RLClass::select(uint8_t ss) {
	reselect(ss);
	...
	if(_rst != 0xff) {
		// DW1000RL data sheet v2.08 §5.6.1 page 20, the RSTn pin should not be driven high but left floating.
		pinMode(_rst, INPUT);
	}
	reset();
...
void DW1000RLClass::reselect(uint8_t ss) {
	_ss = ss;
	pinMode(_ss, OUTPUT);
	digitalWrite(_ss, HIGH);
}

void DW1000RLClass::begin(uint8_t irq, uint8_t rst) {
	...
	// attach interrupt
	//attachInterrupt(_irq, DW1000RLClass::handleInterrupt, CHANGE); // todo interrupt for ESP8266
	// TODO throw error if pin is not a interrupt pin
	attachInterrupt(digitalPinToInterrupt(_irq), DW1000RLClass::handleInterrupt, RISING); // todo interrupt for ESP8266
}

void DW1000RLClass::manageLDE() {
...
void DW1000RLClass::enableClock(byte clock) {
	byte pmscctrl0[LEN_PMSC_CTRL0];
	memset(pmscctrl0, 0, LEN_PMSC_CTRL0);
	readBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, LEN_PMSC_CTRL0);
	if(clock == AUTO_CLOCK) {
...
void DW1000RLClass::enableDebounceClock() {
	byte pmscctrl0[LEN_PMSC_CTRL0];
	memset(pmscctrl0, 0, LEN_PMSC_CTRL0);
	readBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, LEN_PMSC_CTRL0);
...
void DW1000RLClass::enableLedBlinking() {
...
void DW1000RLClass::setGPIOMode(uint8_t msgp, uint8_t mode) {
...
void DW1000RLClass::reset() {
	if(_rst == 0xff) {
		...
		delay(2);  // DW1000RL data sheet v2.08 §5.6.1 page 20: nominal 50ns, to be safe take more time
		pinMode(_rst, INPUT);
...
void DW1000RLClass::enableMode(const byte mode[]) {
	setDataRate(mode[0]);
...
void DW1000RLClass::handleInterrupt() {
...
void DW1000RLClass::getPrintableDeviceIdentifier(char msgBuffer[]) {
...
void DW1000RLClass::getPrintableExtendedUniqueIdentifier(char msgBuffer[]) {
	byte data[LEN_EUI];
...
void DW1000RLClass::getPrintableNetworkIdAndShortAddress(char msgBuffer[]) {
...
void DW1000RLClass::getPrintableDeviceMode(char msgBuffer[]) {
...
/* ###########################################################################
 * #### DW1000RL register read/write ###########################################
 * ######################################################################### */

void DW1000RLClass::readSystemConfigurationRegister() {
	readBytes(SYS_CFG, NO_SUB, _syscfg, LEN_SYS_CFG);
}

void DW1000RLClass::writeSystemConfigurationRegister() {
	writeBytes(SYS_CFG, NO_SUB, _syscfg, LEN_SYS_CFG);
}

void DW1000RLClass::readSystemEventStatusRegister() {
	readBytes(SYS_STATUS, NO_SUB, _sysstatus, LEN_SYS_STATUS);
}

void DW1000RLClass::readNetworkIdAndDeviceAddress() {
	readBytes(PANADR, NO_SUB, _networkAndAddress, LEN_PANADR);
}

void DW1000RLClass::writeNetworkIdAndDeviceAddress() {
	writeBytes(PANADR, NO_SUB, _networkAndAddress, LEN_PANADR);
}

void DW1000RLClass::readSystemEventMaskRegister() {
	readBytes(SYS_MASK, NO_SUB, _sysmask, LEN_SYS_MASK);
}

void DW1000RLClass::writeSystemEventMaskRegister() {
	writeBytes(SYS_MASK, NO_SUB, _sysmask, LEN_SYS_MASK);
}

void DW1000RLClass::readChannelControlRegister() {
	readBytes(CHAN_CTRL, NO_SUB, _chanctrl, LEN_CHAN_CTRL);
}

void DW1000RLClass::writeChannelControlRegister() {
	writeBytes(CHAN_CTRL, NO_SUB, _chanctrl, LEN_CHAN_CTRL);
}

void DW1000RLClass::readTransmitFrameControlRegister() {
	readBytes(TX_FCTRL, NO_SUB, _txfctrl, LEN_TX_FCTRL);
}

void DW1000RLClass::writeTransmitFrameControlRegister() {
	writeBytes(TX_FCTRL, NO_SUB, _txfctrl, LEN_TX_FCTRL);
}

/* ###########################################################################
 * #### DW1000RL operation functions ###########################################
 * ######################################################################### */

void DW1000RLClass::setNetworkId(uint16_t val) {
	_networkAndAddress[2] = (byte)(val & 0xFF);
	_networkAndAddress[3] = (byte)((val >> 8) & 0xFF);
}

void DW1000RLClass::setDeviceAddress(uint16_t val) {
	_networkAndAddress[0] = (byte)(val & 0xFF);
	_networkAndAddress[1] = (byte)((val >> 8) & 0xFF);
}

uint8_t DW1000RLClass::nibbleFromChar(char c) {
	if(c >= '0' && c <= '9') {
		return c-'0';
	}
	if(c >= 'a' && c <= 'f') {
...
void DW1000RLClass::convertToByte(char string[], byte* bytes) {
...
void DW1000RLClass::getTempAndVbat(float& temp, float& vbat) {
...
void DW1000RLClass::setEUI(char eui[]) {
	byte eui_byte[LEN_EUI];
	convertToByte(eui, eui_byte);
	setEUI(eui_byte);
}

void DW1000RLClass::setEUI(byte eui[]) {
...
void DW1000RLClass::setFrameFilter(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFEN_BIT, val);
}

void DW1000RLClass::setFrameFilterBehaveCoordinator(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFBC_BIT, val);
}

void DW1000RLClass::setFrameFilterAllowBeacon(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAB_BIT, val);
}

void DW1000RLClass::setFrameFilterAllowData(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAD_BIT, val);
}

void DW1000RLClass::setFrameFilterAllowAcknowledgement(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAA_BIT, val);
}

void DW1000RLClass::setFrameFilterAllowMAC(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAM_BIT, val);
}

void DW1000RLClass::setFrameFilterAllowReserved(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAR_BIT, val);
}


void DW1000RLClass::setDoubleBuffering(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, DIS_DRXB_BIT, !val);
}

void DW1000RLClass::setInterruptPolarity(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, HIRQ_POL_BIT, val);
}

void DW1000RLClass::setReceiverAutoReenable(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, RXAUTR_BIT, val);
}

void DW1000RLClass::interruptOnSent(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, TXFRS_BIT, val);
}

void DW1000RLClass::interruptOnReceived(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, RXDFR_BIT, val);
	setBit(_sysmask, LEN_SYS_MASK, RXFCG_BIT, val);
}

void DW1000RLClass::interruptOnReceiveFailed(boolean val) {
...
void DW1000RLClass::interruptOnReceiveTimeout(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, RXRFTO_BIT, val);
}

void DW1000RLClass::interruptOnReceiveTimestampAvailable(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, LDEDONE_BIT, val);
}

void DW1000RLClass::interruptOnAutomaticAcknowledgeTrigger(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, AAT_BIT, val);
}

void DW1000RLClass::clearInterrupts() {
	memset(_sysmask, 0, LEN_SYS_MASK);
}

void DW1000RLClass::idle() {
...
void DW1000RLClass::startTransmit() {
...
void DW1000RLClass::newConfiguration() {
	idle();
...
void DW1000RLClass::commitConfiguration() {
...
void DW1000RLClass::waitForResponse(boolean val) {
	setBit(_sysctrl, LEN_SYS_CTRL, WAIT4RESP_BIT, val);
}

void DW1000RLClass::suppressFrameCheck(boolean val) {
	_frameCheck = !val;
}

void DW1000RLClass::useSmartPower(boolean smartPower) {
	_smartPower = smartPower;
	setBit(_syscfg, LEN_SYS_CFG, DIS_STXP_BIT, !smartPower);
}

DW1000Time DW1000RLClass::setDelay(const DW1000Time& delay) {
...
void DW1000RLClass::setDataRate(byte rate) {
	rate &= 0x03;
	_txfctrl[1] &= 0x83;
...
void DW1000RLClass::setPulseFrequency(byte freq) {
	freq &= 0x03;
	_txfctrl[2] &= 0xFC;
...
byte DW1000RLClass::getPulseFrequency() {
	return _pulseFrequency;
}

void DW1000RLClass::setPreambleLength(byte prealen) {
	prealen &= 0x0F;
	_txfctrl[2] &= 0xC3;
...
void DW1000RLClass::useExtendedFrameLength(boolean val) {
	_extendedFrameLength = (val ? FRAME_LENGTH_EXTENDED : FRAME_LENGTH_NORMAL);
	_syscfg[2] &= 0xFC;
	_syscfg[2] |= _extendedFrameLength;
}

void DW1000RLClass::receivePermanently(boolean val) {
...
void DW1000RLClass::setChannel(byte channel) {
	channel &= 0xF;
	_chanctrl[0] = ((channel | (channel << 4)) & 0xFF);
	_channel = channel;
}

void DW1000RLClass::setPreambleCode(byte preacode) {
	preacode &= 0x1F;
	_chanctrl[2] &= 0x3F;
...
void DW1000RLClass::setDefaults() {
	if(_deviceMode == TX_MODE) {
...
void DW1000RLClass::setData(byte data[], uint16_t n) {
	if(_frameCheck) {
...
void DW1000RLClass::setData(const String& data) {
...
// TODO reorder
uint16_t DW1000RLClass::getDataLength() {
	uint16_t len = 0;
	if(_deviceMode == TX_MODE) {
...
void DW1000RLClass::getData(byte data[], uint16_t n) {
	if(n <= 0) {
		return;
	}
...
void DW1000RLClass::getData(String& data) {
	uint16_t i;
...
void DW1000RLClass::correctTimestamp(DW1000Time& timestamp) {
...
boolean DW1000RLClass::isReceiveFailed() {
...
boolean DW1000RLClass::isReceiveTimeout() {
	return getBit(_sysstatus, LEN_SYS_STATUS, RXRFTO_BIT);
}

boolean DW1000RLClass::isClockProblem() {
...
void DW1000RLClass::clearTransmitStatus() {
...
float DW1000RLClass::getReceiveQuality() {
...
float DW1000RLClass::getFirstPathPower() {
...
float DW1000RLClass::getReceivePower() {
 ...
 */
void DW1000RLClass::setBit(byte data[], uint16_t n, uint16_t bit, boolean val) {
	uint16_t idx;
	uint8_t shift;
 ...
 */
boolean DW1000RLClass::getBit(byte data[], uint16_t n, uint16_t bit) {
	uint16_t idx;
	uint8_t  shift;
...
void DW1000RLClass::writeValueToBytes(byte data[], int32_t val, uint16_t n) {
	uint16_t i;
	for(i = 0; i < n; i++) {
...
/*
 * Read bytes from the DW1000RL. Number of bytes depend on register length.
 * @param cmd
 * 		The register address (see Chapter 7 in the DW1000RL user manual).
 * @param data
 ...
 */
// TODO incomplete doc
void DW1000RLClass::readBytes(byte cmd, uint16_t offset, byte data[], uint16_t n) {
	byte header[3];
	uint8_t headerLen = 1;
	uint16_t i = 0;
...
void DW1000RLClass::readBytesOTP(uint16_t address, byte data[]) {
...
void DW1000RLClass::writeByte(byte cmd, uint16_t offset, byte data) {
	writeBytes(cmd, offset, &data, 1);
}

/*
 * Write bytes to the DW1000RL. Single bytes can be written to registers via sub-addressing.
 * @param cmd
 * 		The register address (see Chapter 7 in the DW1000RL user manual).
 * @param offset
...
void DW1000RLClass::writeBytes(byte cmd, uint16_t offset, byte data[], uint16_t data_size) {
	byte header[3];
	uint8_t  headerLen = 1;
	uint16_t  i = 0;
...
void DW1000RLClass::getPrettyBytes(byte data[], char msgBuffer[], uint16_t n) {
	uint16_t i, j, b;
...
void DW1000RLClass::getPrettyBytes(byte cmd, uint16_t offset, char msgBuffer[], uint16_t n) {
	uint16_t i, j, b;
