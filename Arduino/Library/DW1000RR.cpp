 ...
 * Decawave DW1000RR library for arduino.
 *
 ...
 *
 * @file DW1000RR.cpp
 * Arduino driver library (source file) for the Decawave DW1000RR UWB transceiver IC.
 */

#include "DW1000RR.h"

DW1000RRClass DW1000RR;

/* ###########################################################################
 * #### Static member variables ##############################################
 * ######################################################################### */
// pins
uint8_t DW1000RRClass::_ss;
uint8_t DW1000RRClass::_rst;
uint8_t DW1000RRClass::_irq;


// IRQ callbacks
void (* DW1000RRClass::_handleSent)(void)                      = 0;
void (* DW1000RRClass::_handleError)(void)                     = 0;
void (* DW1000RRClass::_handleReceived)(void)                  = 0;
void (* DW1000RRClass::_handleReceiveFailed)(void)             = 0;
void (* DW1000RRClass::_handleReceiveTimeout)(void)            = 0;
void (* DW1000RRClass::_handleReceiveTimestampAvailable)(void) = 0;

// registers
byte       DW1000RRClass::_syscfg[LEN_SYS_CFG];
byte       DW1000RRClass::_sysctrl[LEN_SYS_CTRL];
byte       DW1000RRClass::_sysstatus[LEN_SYS_STATUS];
byte       DW1000RRClass::_txfctrl[LEN_TX_FCTRL];
byte       DW1000RRClass::_sysmask[LEN_SYS_MASK];
byte       DW1000RRClass::_chanctrl[LEN_CHAN_CTRL];
byte       DW1000RRClass::_networkAndAddress[LEN_PANADR];

// monitoring
byte DW1000RRClass::_vmeas3v3 = 0;
byte DW1000RRClass::_tmeas23C = 0;

// driver internal state
byte       DW1000RRClass::_extendedFrameLength = FRAME_LENGTH_NORMAL;
byte       DW1000RRClass::_pacSize             = PAC_SIZE_8;
byte       DW1000RRClass::_pulseFrequency      = TX_PULSE_FREQ_16MHZ;
byte       DW1000RRClass::_dataRate            = TRX_RATE_6800KBPS;
byte       DW1000RRClass::_preambleLength      = TX_PREAMBLE_LEN_128;
byte       DW1000RRClass::_preambleCode        = PREAMBLE_CODE_16MHZ_4;
byte       DW1000RRClass::_channel             = CHANNEL_5;
DW1000Time DW1000RRClass::_antennaDelay;
boolean    DW1000RRClass::_smartPower          = false;

boolean    DW1000RRClass::_frameCheck          = true;
boolean    DW1000RRClass::_permanentReceive    = false;
uint8_t    DW1000RRClass::_deviceMode          = IDLE_MODE; // TODO replace by enum
...
constexpr byte DW1000RRClass::MODE_LONGDATA_RANGE_LOWPOWER[];
constexpr byte DW1000RRClass::MODE_SHORTDATA_FAST_LOWPOWER[];
constexpr byte DW1000RRClass::MODE_LONGDATA_FAST_LOWPOWER[];
constexpr byte DW1000RRClass::MODE_SHORTDATA_FAST_ACCURACY[];
constexpr byte DW1000RRClass::MODE_LONGDATA_FAST_ACCURACY[];
constexpr byte DW1000RRClass::MODE_LONGDATA_RANGE_ACCURACY[];
/*
const byte DW1000RRClass::MODE_LONGDATA_RANGE_LOWPOWER[] = {TRX_RATE_110KBPS, TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_2048};
const byte DW1000RRClass::MODE_SHORTDATA_FAST_LOWPOWER[] = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_128};
const byte DW1000RRClass::MODE_LONGDATA_FAST_LOWPOWER[]  = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_1024};
const byte DW1000RRClass::MODE_SHORTDATA_FAST_ACCURACY[] = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_128};
const byte DW1000RRClass::MODE_LONGDATA_FAST_ACCURACY[]  = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_1024};
const byte DW1000RRClass::MODE_LONGDATA_RANGE_ACCURACY[] = {TRX_RATE_110KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_2048};
*/
// range bias tables (500 MHz in [mm] and 900 MHz in [2mm] - to fit into bytes)
constexpr byte DW1000RRClass::BIAS_500_16[];
constexpr byte DW1000RRClass::BIAS_500_64[];
constexpr byte DW1000RRClass::BIAS_900_16[];
constexpr byte DW1000RRClass::BIAS_900_64[];
/*
const byte DW1000RRClass::BIAS_500_16[] = {198, 187, 179, 163, 143, 127, 109, 84, 59, 31, 0, 36, 65, 84, 97, 106, 110, 112};
const byte DW1000RRClass::BIAS_500_64[] = {110, 105, 100, 93, 82, 69, 51, 27, 0, 21, 35, 42, 49, 62, 71, 76, 81, 86};
const byte DW1000RRClass::BIAS_900_16[] = {137, 122, 105, 88, 69, 47, 25, 0, 21, 48, 79, 105, 127, 147, 160, 169, 178, 197};
const byte DW1000RRClass::BIAS_900_64[] = {147, 133, 117, 99, 75, 50, 29, 0, 24, 45, 63, 76, 87, 98, 116, 122, 132, 142};
*/
// SPI settings
const SPISettings DW1000RRClass::_fastSPI = SPISettings(20000000L, MSBFIRST, SPI_MODE0);
const SPISettings DW1000RRClass::_slowSPI = SPISettings(3000000L, MSBFIRST, SPI_MODE0);
const SPISettings* DW1000RRClass::_currentSPI = &_fastSPI;

/* ###########################################################################
 * #### Init and end #######################################################
 * ######################################################################### */

void DW1000RRClass::end() {
	SPI.end();
}

void DW1000RRClass::select(uint8_t ss) {
	reselect(ss);
	...
	if(_rst != 0xff) {
		// DW1000RR data sheet v2.08 §5.6.1 page 20, the RSTn pin should not be driven high but left floating.
		pinMode(_rst, INPUT);
	}
	reset();
...
void DW1000RRClass::reselect(uint8_t ss) {
	_ss = ss;
	pinMode(_ss, OUTPUT);
	digitalWrite(_ss, HIGH);
}

void DW1000RRClass::begin(uint8_t irq, uint8_t rst) {
	...
	// attach interrupt
	//attachInterrupt(_irq, DW1000RRClass::handleInterrupt, CHANGE); // todo interrupt for ESP8266
	// TODO throw error if pin is not a interrupt pin
	attachInterrupt(digitalPinToInterrupt(_irq), DW1000RRClass::handleInterrupt, RISING); // todo interrupt for ESP8266
}

void DW1000RRClass::manageLDE() {
...
void DW1000RRClass::enableClock(byte clock) {
	byte pmscctrl0[LEN_PMSC_CTRL0];
	memset(pmscctrl0, 0, LEN_PMSC_CTRL0);
	readBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, LEN_PMSC_CTRL0);
	if(clock == AUTO_CLOCK) {
...
void DW1000RRClass::enableDebounceClock() {
	byte pmscctrl0[LEN_PMSC_CTRL0];
	memset(pmscctrl0, 0, LEN_PMSC_CTRL0);
	readBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, LEN_PMSC_CTRL0);
...
void DW1000RRClass::enableLedBlinking() {
...
void DW1000RRClass::setGPIOMode(uint8_t msgp, uint8_t mode) {
...
void DW1000RRClass::reset() {
	if(_rst == 0xff) {
		...
		delay(2);  // DW1000RR data sheet v2.08 §5.6.1 page 20: nominal 50ns, to be safe take more time
		pinMode(_rst, INPUT);
...
void DW1000RRClass::enableMode(const byte mode[]) {
	setDataRate(mode[0]);
...
void DW1000RRClass::handleInterrupt() {
...
void DW1000RRClass::getPrintableDeviceIdentifier(char msgBuffer[]) {
...
void DW1000RRClass::getPrintableExtendedUniqueIdentifier(char msgBuffer[]) {
	byte data[LEN_EUI];
...
void DW1000RRClass::getPrintableNetworkIdAndShortAddress(char msgBuffer[]) {
...
void DW1000RRClass::getPrintableDeviceMode(char msgBuffer[]) {
...
/* ###########################################################################
 * #### DW1000RR register read/write ###########################################
 * ######################################################################### */

void DW1000RRClass::readSystemConfigurationRegister() {
	readBytes(SYS_CFG, NO_SUB, _syscfg, LEN_SYS_CFG);
}

void DW1000RRClass::writeSystemConfigurationRegister() {
	writeBytes(SYS_CFG, NO_SUB, _syscfg, LEN_SYS_CFG);
}

void DW1000RRClass::readSystemEventStatusRegister() {
	readBytes(SYS_STATUS, NO_SUB, _sysstatus, LEN_SYS_STATUS);
}

void DW1000RRClass::readNetworkIdAndDeviceAddress() {
	readBytes(PANADR, NO_SUB, _networkAndAddress, LEN_PANADR);
}

void DW1000RRClass::writeNetworkIdAndDeviceAddress() {
	writeBytes(PANADR, NO_SUB, _networkAndAddress, LEN_PANADR);
}

void DW1000RRClass::readSystemEventMaskRegister() {
	readBytes(SYS_MASK, NO_SUB, _sysmask, LEN_SYS_MASK);
}

void DW1000RRClass::writeSystemEventMaskRegister() {
	writeBytes(SYS_MASK, NO_SUB, _sysmask, LEN_SYS_MASK);
}

void DW1000RRClass::readChannelControlRegister() {
	readBytes(CHAN_CTRL, NO_SUB, _chanctrl, LEN_CHAN_CTRL);
}

void DW1000RRClass::writeChannelControlRegister() {
	writeBytes(CHAN_CTRL, NO_SUB, _chanctrl, LEN_CHAN_CTRL);
}

void DW1000RRClass::readTransmitFrameControlRegister() {
	readBytes(TX_FCTRL, NO_SUB, _txfctrl, LEN_TX_FCTRL);
}

void DW1000RRClass::writeTransmitFrameControlRegister() {
	writeBytes(TX_FCTRL, NO_SUB, _txfctrl, LEN_TX_FCTRL);
}

/* ###########################################################################
 * #### DW1000RR operation functions ###########################################
 * ######################################################################### */

void DW1000RRClass::setNetworkId(uint16_t val) {
	_networkAndAddress[2] = (byte)(val & 0xFF);
	_networkAndAddress[3] = (byte)((val >> 8) & 0xFF);
}

void DW1000RRClass::setDeviceAddress(uint16_t val) {
	_networkAndAddress[0] = (byte)(val & 0xFF);
	_networkAndAddress[1] = (byte)((val >> 8) & 0xFF);
}

uint8_t DW1000RRClass::nibbleFromChar(char c) {
	if(c >= '0' && c <= '9') {
		return c-'0';
	}
	if(c >= 'a' && c <= 'f') {
...
void DW1000RRClass::convertToByte(char string[], byte* bytes) {
...
void DW1000RRClass::getTempAndVbat(float& temp, float& vbat) {
...
void DW1000RRClass::setEUI(char eui[]) {
	byte eui_byte[LEN_EUI];
	convertToByte(eui, eui_byte);
	setEUI(eui_byte);
}

void DW1000RRClass::setEUI(byte eui[]) {
...
void DW1000RRClass::setFrameFilter(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFEN_BIT, val);
}

void DW1000RRClass::setFrameFilterBehaveCoordinator(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFBC_BIT, val);
}

void DW1000RRClass::setFrameFilterAllowBeacon(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAB_BIT, val);
}

void DW1000RRClass::setFrameFilterAllowData(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAD_BIT, val);
}

void DW1000RRClass::setFrameFilterAllowAcknowledgement(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAA_BIT, val);
}

void DW1000RRClass::setFrameFilterAllowMAC(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAM_BIT, val);
}

void DW1000RRClass::setFrameFilterAllowReserved(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAR_BIT, val);
}


void DW1000RRClass::setDoubleBuffering(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, DIS_DRXB_BIT, !val);
}

void DW1000RRClass::setInterruptPolarity(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, HIRQ_POL_BIT, val);
}

void DW1000RRClass::setReceiverAutoReenable(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, RXAUTR_BIT, val);
}

void DW1000RRClass::interruptOnSent(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, TXFRS_BIT, val);
}

void DW1000RRClass::interruptOnReceived(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, RXDFR_BIT, val);
	setBit(_sysmask, LEN_SYS_MASK, RXFCG_BIT, val);
}

void DW1000RRClass::interruptOnReceiveFailed(boolean val) {
...
void DW1000RRClass::interruptOnReceiveTimeout(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, RXRFTO_BIT, val);
}

void DW1000RRClass::interruptOnReceiveTimestampAvailable(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, LDEDONE_BIT, val);
}

void DW1000RRClass::interruptOnAutomaticAcknowledgeTrigger(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, AAT_BIT, val);
}

void DW1000RRClass::clearInterrupts() {
	memset(_sysmask, 0, LEN_SYS_MASK);
}

void DW1000RRClass::idle() {
...
void DW1000RRClass::startTransmit() {
...
void DW1000RRClass::newConfiguration() {
	idle();
...
void DW1000RRClass::commitConfiguration() {
...
void DW1000RRClass::waitForResponse(boolean val) {
	setBit(_sysctrl, LEN_SYS_CTRL, WAIT4RESP_BIT, val);
}

void DW1000RRClass::suppressFrameCheck(boolean val) {
	_frameCheck = !val;
}

void DW1000RRClass::useSmartPower(boolean smartPower) {
	_smartPower = smartPower;
	setBit(_syscfg, LEN_SYS_CFG, DIS_STXP_BIT, !smartPower);
}

DW1000Time DW1000RRClass::setDelay(const DW1000Time& delay) {
...
void DW1000RRClass::setDataRate(byte rate) {
	rate &= 0x03;
	_txfctrl[1] &= 0x83;
...
void DW1000RRClass::setPulseFrequency(byte freq) {
	freq &= 0x03;
	_txfctrl[2] &= 0xFC;
...
byte DW1000RRClass::getPulseFrequency() {
	return _pulseFrequency;
}

void DW1000RRClass::setPreambleLength(byte prealen) {
	prealen &= 0x0F;
	_txfctrl[2] &= 0xC3;
...
void DW1000RRClass::useExtendedFrameLength(boolean val) {
	_extendedFrameLength = (val ? FRAME_LENGTH_EXTENDED : FRAME_LENGTH_NORMAL);
	_syscfg[2] &= 0xFC;
	_syscfg[2] |= _extendedFrameLength;
}

void DW1000RRClass::receivePermanently(boolean val) {
...
void DW1000RRClass::setChannel(byte channel) {
	channel &= 0xF;
	_chanctrl[0] = ((channel | (channel << 4)) & 0xFF);
	_channel = channel;
}

void DW1000RRClass::setPreambleCode(byte preacode) {
	preacode &= 0x1F;
	_chanctrl[2] &= 0x3F;
...
void DW1000RRClass::setDefaults() {
	if(_deviceMode == TX_MODE) {
...
void DW1000RRClass::setData(byte data[], uint16_t n) {
	if(_frameCheck) {
...
void DW1000RRClass::setData(const String& data) {
...
// TODO reorder
uint16_t DW1000RRClass::getDataLength() {
	uint16_t len = 0;
	if(_deviceMode == TX_MODE) {
...
void DW1000RRClass::getData(byte data[], uint16_t n) {
	if(n <= 0) {
		return;
	}
...
void DW1000RRClass::getData(String& data) {
	uint16_t i;
...
void DW1000RRClass::correctTimestamp(DW1000Time& timestamp) {
...
boolean DW1000RRClass::isReceiveFailed() {
...
boolean DW1000RRClass::isReceiveTimeout() {
	return getBit(_sysstatus, LEN_SYS_STATUS, RXRFTO_BIT);
}

boolean DW1000RRClass::isClockProblem() {
...
void DW1000RRClass::clearTransmitStatus() {
...
float DW1000RRClass::getReceiveQuality() {
...
float DW1000RRClass::getFirstPathPower() {
...
float DW1000RRClass::getReceivePower() {
 ...
 */
void DW1000RRClass::setBit(byte data[], uint16_t n, uint16_t bit, boolean val) {
	uint16_t idx;
	uint8_t shift;
 ...
 */
boolean DW1000RRClass::getBit(byte data[], uint16_t n, uint16_t bit) {
	uint16_t idx;
	uint8_t  shift;
...
void DW1000RRClass::writeValueToBytes(byte data[], int32_t val, uint16_t n) {
	uint16_t i;
	for(i = 0; i < n; i++) {
...
/*
 * Read bytes from the DW1000RR. Number of bytes depend on register length.
 * @param cmd
 * 		The register address (see Chapter 7 in the DW1000RR user manual).
 * @param data
 ...
 */
// TODO incomplete doc
void DW1000RRClass::readBytes(byte cmd, uint16_t offset, byte data[], uint16_t n) {
	byte header[3];
	uint8_t headerLen = 1;
	uint16_t i = 0;
...
void DW1000RRClass::readBytesOTP(uint16_t address, byte data[]) {
...
void DW1000RRClass::writeByte(byte cmd, uint16_t offset, byte data) {
	writeBytes(cmd, offset, &data, 1);
}

/*
 * Write bytes to the DW1000RR. Single bytes can be written to registers via sub-addressing.
 * @param cmd
 * 		The register address (see Chapter 7 in the DW1000RR user manual).
 * @param offset
...
void DW1000RRClass::writeBytes(byte cmd, uint16_t offset, byte data[], uint16_t data_size) {
	byte header[3];
	uint8_t  headerLen = 1;
	uint16_t  i = 0;
...
void DW1000RRClass::getPrettyBytes(byte data[], char msgBuffer[], uint16_t n) {
	uint16_t i, j, b;
...
void DW1000RRClass::getPrettyBytes(byte cmd, uint16_t offset, char msgBuffer[], uint16_t n) {
	uint16_t i, j, b;
