 ...
 * Decawave DW1000FL library for arduino.
 *
 ...
 *
 * @file DW1000FL.cpp
 * Arduino driver library (source file) for the Decawave DW1000FL UWB transceiver IC.
 */

#include "DW1000FL.h"

DW1000FLClass DW1000FL;

/* ###########################################################################
 * #### Static member variables ##############################################
 * ######################################################################### */
// pins
uint8_t DW1000FLClass::_ss;
uint8_t DW1000FLClass::_rst;
uint8_t DW1000FLClass::_irq;


// IRQ callbacks
void (* DW1000FLClass::_handleSent)(void)                      = 0;
void (* DW1000FLClass::_handleError)(void)                     = 0;
void (* DW1000FLClass::_handleReceived)(void)                  = 0;
void (* DW1000FLClass::_handleReceiveFailed)(void)             = 0;
void (* DW1000FLClass::_handleReceiveTimeout)(void)            = 0;
void (* DW1000FLClass::_handleReceiveTimestampAvailable)(void) = 0;

// registers
byte       DW1000FLClass::_syscfg[LEN_SYS_CFG];
byte       DW1000FLClass::_sysctrl[LEN_SYS_CTRL];
byte       DW1000FLClass::_sysstatus[LEN_SYS_STATUS];
byte       DW1000FLClass::_txfctrl[LEN_TX_FCTRL];
byte       DW1000FLClass::_sysmask[LEN_SYS_MASK];
byte       DW1000FLClass::_chanctrl[LEN_CHAN_CTRL];
byte       DW1000FLClass::_networkAndAddress[LEN_PANADR];

// monitoring
byte DW1000FLClass::_vmeas3v3 = 0;
byte DW1000FLClass::_tmeas23C = 0;

// driver internal state
byte       DW1000FLClass::_extendedFrameLength = FRAME_LENGTH_NORMAL;
byte       DW1000FLClass::_pacSize             = PAC_SIZE_8;
byte       DW1000FLClass::_pulseFrequency      = TX_PULSE_FREQ_16MHZ;
byte       DW1000FLClass::_dataRate            = TRX_RATE_6800KBPS;
byte       DW1000FLClass::_preambleLength      = TX_PREAMBLE_LEN_128;
byte       DW1000FLClass::_preambleCode        = PREAMBLE_CODE_16MHZ_4;
byte       DW1000FLClass::_channel             = CHANNEL_5;
DW1000Time DW1000FLClass::_antennaDelay;
boolean    DW1000FLClass::_smartPower          = false;

boolean    DW1000FLClass::_frameCheck          = true;
boolean    DW1000FLClass::_permanentReceive    = false;
uint8_t    DW1000FLClass::_deviceMode          = IDLE_MODE; // TODO replace by enum
...
constexpr byte DW1000FLClass::MODE_LONGDATA_RANGE_LOWPOWER[];
constexpr byte DW1000FLClass::MODE_SHORTDATA_FAST_LOWPOWER[];
constexpr byte DW1000FLClass::MODE_LONGDATA_FAST_LOWPOWER[];
constexpr byte DW1000FLClass::MODE_SHORTDATA_FAST_ACCURACY[];
constexpr byte DW1000FLClass::MODE_LONGDATA_FAST_ACCURACY[];
constexpr byte DW1000FLClass::MODE_LONGDATA_RANGE_ACCURACY[];
/*
const byte DW1000FLClass::MODE_LONGDATA_RANGE_LOWPOWER[] = {TRX_RATE_110KBPS, TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_2048};
const byte DW1000FLClass::MODE_SHORTDATA_FAST_LOWPOWER[] = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_128};
const byte DW1000FLClass::MODE_LONGDATA_FAST_LOWPOWER[]  = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_1024};
const byte DW1000FLClass::MODE_SHORTDATA_FAST_ACCURACY[] = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_128};
const byte DW1000FLClass::MODE_LONGDATA_FAST_ACCURACY[]  = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_1024};
const byte DW1000FLClass::MODE_LONGDATA_RANGE_ACCURACY[] = {TRX_RATE_110KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_2048};
*/
// range bias tables (500 MHz in [mm] and 900 MHz in [2mm] - to fit into bytes)
constexpr byte DW1000FLClass::BIAS_500_16[];
constexpr byte DW1000FLClass::BIAS_500_64[];
constexpr byte DW1000FLClass::BIAS_900_16[];
constexpr byte DW1000FLClass::BIAS_900_64[];
/*
const byte DW1000FLClass::BIAS_500_16[] = {198, 187, 179, 163, 143, 127, 109, 84, 59, 31, 0, 36, 65, 84, 97, 106, 110, 112};
const byte DW1000FLClass::BIAS_500_64[] = {110, 105, 100, 93, 82, 69, 51, 27, 0, 21, 35, 42, 49, 62, 71, 76, 81, 86};
const byte DW1000FLClass::BIAS_900_16[] = {137, 122, 105, 88, 69, 47, 25, 0, 21, 48, 79, 105, 127, 147, 160, 169, 178, 197};
const byte DW1000FLClass::BIAS_900_64[] = {147, 133, 117, 99, 75, 50, 29, 0, 24, 45, 63, 76, 87, 98, 116, 122, 132, 142};
*/
// SPI settings
const SPISettings DW1000FLClass::_fastSPI = SPISettings(20000000L, MSBFIRST, SPI_MODE0);
const SPISettings DW1000FLClass::_slowSPI = SPISettings(3000000L, MSBFIRST, SPI_MODE0);
const SPISettings* DW1000FLClass::_currentSPI = &_fastSPI;

/* ###########################################################################
 * #### Init and end #######################################################
 * ######################################################################### */

void DW1000FLClass::end() {
	SPI.end();
}

void DW1000FLClass::select(uint8_t ss) {
	reselect(ss);
	...
	if(_rst != 0xff) {
		// DW1000FL data sheet v2.08 §5.6.1 page 20, the RSTn pin should not be driven high but left floating.
		pinMode(_rst, INPUT);
	}
	reset();
...
void DW1000FLClass::reselect(uint8_t ss) {
	_ss = ss;
	pinMode(_ss, OUTPUT);
	digitalWrite(_ss, HIGH);
}

void DW1000FLClass::begin(uint8_t irq, uint8_t rst) {
	...
	// attach interrupt
	//attachInterrupt(_irq, DW1000FLClass::handleInterrupt, CHANGE); // todo interrupt for ESP8266
	// TODO throw error if pin is not a interrupt pin
	attachInterrupt(digitalPinToInterrupt(_irq), DW1000FLClass::handleInterrupt, RISING); // todo interrupt for ESP8266
}

void DW1000FLClass::manageLDE() {
...
void DW1000FLClass::enableClock(byte clock) {
	byte pmscctrl0[LEN_PMSC_CTRL0];
	memset(pmscctrl0, 0, LEN_PMSC_CTRL0);
	readBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, LEN_PMSC_CTRL0);
	if(clock == AUTO_CLOCK) {
...
void DW1000FLClass::enableDebounceClock() {
	byte pmscctrl0[LEN_PMSC_CTRL0];
	memset(pmscctrl0, 0, LEN_PMSC_CTRL0);
	readBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, LEN_PMSC_CTRL0);
...
void DW1000FLClass::enableLedBlinking() {
	...
	setGPIOMode(MSGP2, LED_MODE );
    setGPIOMode(MSGP3, LED_MODE );
}

void DW1000FLClass::setGPIOMode(uint8_t msgp, uint8_t mode) {
...
void DW1000FLClass::reset() {
	if(_rst == 0xff) {
		...
		delay(2);  // DW1000FL data sheet v2.08 §5.6.1 page 20: nominal 50ns, to be safe take more time
		pinMode(_rst, INPUT);
...
void DW1000FLClass::enableMode(const byte mode[]) {
	setDataRate(mode[0]);
...
void DW1000FLClass::tune() {
	...
	if(_preambleLength == TX_PREAMBLE_LEN_1536 || _preambleLength == TX_PREAMBLE_LEN_2048 ||
		 _preambleLength == TX_PREAMBLE_LEN_1024 || _preambleLength == TX_PREAMBLE_LEN_4096) {
		if(_dataRate == TRX_RATE_110KBPS) {
...
void DW1000FLClass::handleInterrupt() {
...
void DW1000FLClass::getPrintableDeviceIdentifier(char msgBuffer[]) {
...
void DW1000FLClass::getPrintableExtendedUniqueIdentifier(char msgBuffer[]) {
	byte data[LEN_EUI];
...
void DW1000FLClass::getPrintableNetworkIdAndShortAddress(char msgBuffer[]) {
...
void DW1000FLClass::getPrintableDeviceMode(char msgBuffer[]) {
...
/* ###########################################################################
 * #### DW1000FL register read/write ###########################################
 * ######################################################################### */

void DW1000FLClass::readSystemConfigurationRegister() {
	readBytes(SYS_CFG, NO_SUB, _syscfg, LEN_SYS_CFG);
}

void DW1000FLClass::writeSystemConfigurationRegister() {
	writeBytes(SYS_CFG, NO_SUB, _syscfg, LEN_SYS_CFG);
}

void DW1000FLClass::readSystemEventStatusRegister() {
	readBytes(SYS_STATUS, NO_SUB, _sysstatus, LEN_SYS_STATUS);
}

void DW1000FLClass::readNetworkIdAndDeviceAddress() {
	readBytes(PANADR, NO_SUB, _networkAndAddress, LEN_PANADR);
}

void DW1000FLClass::writeNetworkIdAndDeviceAddress() {
	writeBytes(PANADR, NO_SUB, _networkAndAddress, LEN_PANADR);
}

void DW1000FLClass::readSystemEventMaskRegister() {
	readBytes(SYS_MASK, NO_SUB, _sysmask, LEN_SYS_MASK);
}

void DW1000FLClass::writeSystemEventMaskRegister() {
	writeBytes(SYS_MASK, NO_SUB, _sysmask, LEN_SYS_MASK);
}

void DW1000FLClass::readChannelControlRegister() {
	readBytes(CHAN_CTRL, NO_SUB, _chanctrl, LEN_CHAN_CTRL);
}

void DW1000FLClass::writeChannelControlRegister() {
	writeBytes(CHAN_CTRL, NO_SUB, _chanctrl, LEN_CHAN_CTRL);
}

void DW1000FLClass::readTransmitFrameControlRegister() {
	readBytes(TX_FCTRL, NO_SUB, _txfctrl, LEN_TX_FCTRL);
}

void DW1000FLClass::writeTransmitFrameControlRegister() {
	writeBytes(TX_FCTRL, NO_SUB, _txfctrl, LEN_TX_FCTRL);
}

/* ###########################################################################
 * #### DW1000FL operation functions ###########################################
 * ######################################################################### */

void DW1000FLClass::setNetworkId(uint16_t val) {
	_networkAndAddress[2] = (byte)(val & 0xFF);
	_networkAndAddress[3] = (byte)((val >> 8) & 0xFF);
}

void DW1000FLClass::setDeviceAddress(uint16_t val) {
	_networkAndAddress[0] = (byte)(val & 0xFF);
	_networkAndAddress[1] = (byte)((val >> 8) & 0xFF);
}

uint8_t DW1000FLClass::nibbleFromChar(char c) {
	if(c >= '0' && c <= '9') {
		return c-'0';
	}
	if(c >= 'a' && c <= 'f') {
...
void DW1000FLClass::convertToByte(char string[], byte* bytes) {
...
void DW1000FLClass::getTempAndVbat(float& temp, float& vbat) {
...
void DW1000FLClass::setEUI(char eui[]) {
	byte eui_byte[LEN_EUI];
	convertToByte(eui, eui_byte);
	setEUI(eui_byte);
}

void DW1000FLClass::setEUI(byte eui[]) {
...
void DW1000FLClass::setFrameFilter(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFEN_BIT, val);
}

void DW1000FLClass::setFrameFilterBehaveCoordinator(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFBC_BIT, val);
}

void DW1000FLClass::setFrameFilterAllowBeacon(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAB_BIT, val);
}

void DW1000FLClass::setFrameFilterAllowData(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAD_BIT, val);
}

void DW1000FLClass::setFrameFilterAllowAcknowledgement(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAA_BIT, val);
}

void DW1000FLClass::setFrameFilterAllowMAC(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAM_BIT, val);
}

void DW1000FLClass::setFrameFilterAllowReserved(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAR_BIT, val);
}


void DW1000FLClass::setDoubleBuffering(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, DIS_DRXB_BIT, !val);
}

void DW1000FLClass::setInterruptPolarity(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, HIRQ_POL_BIT, val);
}

void DW1000FLClass::setReceiverAutoReenable(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, RXAUTR_BIT, val);
}

void DW1000FLClass::interruptOnSent(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, TXFRS_BIT, val);
}

void DW1000FLClass::interruptOnReceived(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, RXDFR_BIT, val);
	setBit(_sysmask, LEN_SYS_MASK, RXFCG_BIT, val);
}

void DW1000FLClass::interruptOnReceiveFailed(boolean val) {
...
void DW1000FLClass::interruptOnReceiveTimeout(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, RXRFTO_BIT, val);
}

void DW1000FLClass::interruptOnReceiveTimestampAvailable(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, LDEDONE_BIT, val);
}

void DW1000FLClass::interruptOnAutomaticAcknowledgeTrigger(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, AAT_BIT, val);
}

void DW1000FLClass::clearInterrupts() {
	memset(_sysmask, 0, LEN_SYS_MASK);
}

void DW1000FLClass::idle() {
...
void DW1000FLClass::startTransmit() {
...
void DW1000FLClass::newConfiguration() {
	idle();
...
void DW1000FLClass::commitConfiguration() {
...
void DW1000FLClass::waitForResponse(boolean val) {
	setBit(_sysctrl, LEN_SYS_CTRL, WAIT4RESP_BIT, val);
}

void DW1000FLClass::suppressFrameCheck(boolean val) {
	_frameCheck = !val;
}

void DW1000FLClass::useSmartPower(boolean smartPower) {
	_smartPower = smartPower;
	setBit(_syscfg, LEN_SYS_CFG, DIS_STXP_BIT, !smartPower);
}

DW1000Time DW1000FLClass::setDelay(const DW1000Time& delay) {
...
void DW1000FLClass::setDataRate(byte rate) {
	rate &= 0x03;
	_txfctrl[1] &= 0x83;
...
void DW1000FLClass::setPulseFrequency(byte freq) {
	freq &= 0x03;
	_txfctrl[2] &= 0xFC;
...
byte DW1000FLClass::getPulseFrequency() {
	return _pulseFrequency;
}

void DW1000FLClass::setPreambleLength(byte prealen) {
	prealen &= 0x0F;
	_txfctrl[2] &= 0xC3;
...
void DW1000FLClass::useExtendedFrameLength(boolean val) {
	_extendedFrameLength = (val ? FRAME_LENGTH_EXTENDED : FRAME_LENGTH_NORMAL);
	_syscfg[2] &= 0xFC;
	_syscfg[2] |= _extendedFrameLength;
}

void DW1000FLClass::receivePermanently(boolean val) {
...
void DW1000FLClass::setChannel(byte channel) {
	channel &= 0xF;
	_chanctrl[0] = ((channel | (channel << 4)) & 0xFF);
	_channel = channel;
}

void DW1000FLClass::setPreambleCode(byte preacode) {
	preacode &= 0x1F;
	_chanctrl[2] &= 0x3F;
...
void DW1000FLClass::setDefaults() {
	if(_deviceMode == TX_MODE) {
...
void DW1000FLClass::setData(byte data[], uint16_t n) {
	if(_frameCheck) {
...
void DW1000FLClass::setData(const String& data) {
...
// TODO reorder
uint16_t DW1000FLClass::getDataLength() {
	uint16_t len = 0;
	if(_deviceMode == TX_MODE) {
...
void DW1000FLClass::getData(byte data[], uint16_t n) {
	if(n <= 0) {
		return;
	}
...
void DW1000FLClass::getData(String& data) {
	uint16_t i;
...
void DW1000FLClass::correctTimestamp(DW1000Time& timestamp) {
...
boolean DW1000FLClass::isReceiveFailed() {
...
boolean DW1000FLClass::isReceiveTimeout() {
	return getBit(_sysstatus, LEN_SYS_STATUS, RXRFTO_BIT);
}

boolean DW1000FLClass::isClockProblem() {
...
void DW1000FLClass::clearTransmitStatus() {
...
float DW1000FLClass::getReceiveQuality() {
...
float DW1000FLClass::getFirstPathPower() {
...
float DW1000FLClass::getReceivePower() {
 ...
 */
void DW1000FLClass::setBit(byte data[], uint16_t n, uint16_t bit, boolean val) {
	uint16_t idx;
	uint8_t shift;
 ...
 */
boolean DW1000FLClass::getBit(byte data[], uint16_t n, uint16_t bit) {
	uint16_t idx;
	uint8_t  shift;
...
void DW1000FLClass::writeValueToBytes(byte data[], int32_t val, uint16_t n) {
	uint16_t i;
	for(i = 0; i < n; i++) {
...
/*
 * Read bytes from the DW1000FL. Number of bytes depend on register length.
 * @param cmd
 * 		The register address (see Chapter 7 in the DW1000FL user manual).
 * @param data
 ...
 */
// TODO incomplete doc
void DW1000FLClass::readBytes(byte cmd, uint16_t offset, byte data[], uint16_t n) {
	byte header[3];
	uint8_t headerLen = 1;
	uint16_t i = 0;
...
void DW1000FLClass::readBytesOTP(uint16_t address, byte data[]) {
...
void DW1000FLClass::writeByte(byte cmd, uint16_t offset, byte data) {
	writeBytes(cmd, offset, &data, 1);
}

/*
 * Write bytes to the DW1000FL. Single bytes can be written to registers via sub-addressing.
 * @param cmd
 * 		The register address (see Chapter 7 in the DW1000FL user manual).
 * @param offset
...
void DW1000FLClass::writeBytes(byte cmd, uint16_t offset, byte data[], uint16_t data_size) {
	byte header[3];
	uint8_t  headerLen = 1;
	uint16_t  i = 0;
...
void DW1000FLClass::getPrettyBytes(byte data[], char msgBuffer[], uint16_t n) {
	uint16_t i, j, b;
...
void DW1000FLClass::getPrettyBytes(byte cmd, uint16_t offset, char msgBuffer[], uint16_t n) {
	uint16_t i, j, b;
