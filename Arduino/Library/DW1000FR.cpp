...
DW1000FRClass DW1000FR;

/* ###########################################################################
 * #### Static member variables ##############################################
 * ######################################################################### */
// pins
uint8_t DW1000FRClass::_ss;
uint8_t DW1000FRClass::_rst;
uint8_t DW1000FRClass::_irq;


// IRQ callbacks
void (* DW1000FRClass::_handleSent)(void)                      = 0;
void (* DW1000FRClass::_handleError)(void)                     = 0;
void (* DW1000FRClass::_handleReceived)(void)                  = 0;
void (* DW1000FRClass::_handleReceiveFailed)(void)             = 0;
void (* DW1000FRClass::_handleReceiveTimeout)(void)            = 0;
void (* DW1000FRClass::_handleReceiveTimestampAvailable)(void) = 0;

// registers
byte       DW1000FRClass::_syscfg[LEN_SYS_CFG];
byte       DW1000FRClass::_sysctrl[LEN_SYS_CTRL];
byte       DW1000FRClass::_sysstatus[LEN_SYS_STATUS];
byte       DW1000FRClass::_txfctrl[LEN_TX_FCTRL];
byte       DW1000FRClass::_sysmask[LEN_SYS_MASK];
byte       DW1000FRClass::_chanctrl[LEN_CHAN_CTRL];
byte       DW1000FRClass::_networkAndAddress[LEN_PANADR];

// monitoring
byte DW1000FRClass::_vmeas3v3 = 0;
byte DW1000FRClass::_tmeas23C = 0;

// driver internal state
byte       DW1000FRClass::_extendedFrameLength = FRAME_LENGTH_NORMAL;
byte       DW1000FRClass::_pacSize             = PAC_SIZE_8;
byte       DW1000FRClass::_pulseFrequency      = TX_PULSE_FREQ_16MHZ;
byte       DW1000FRClass::_dataRate            = TRX_RATE_6800KBPS;
byte       DW1000FRClass::_preambleLength      = TX_PREAMBLE_LEN_128;
byte       DW1000FRClass::_preambleCode        = PREAMBLE_CODE_16MHZ_4;
byte       DW1000FRClass::_channel             = CHANNEL_5;
DW1000Time DW1000FRClass::_antennaDelay;
boolean    DW1000FRClass::_smartPower          = false;

boolean    DW1000FRClass::_frameCheck          = true;
boolean    DW1000FRClass::_permanentReceive    = false;
uint8_t    DW1000FRClass::_deviceMode          = IDLE_MODE; // TODO replace by enum
...
constexpr byte DW1000FRClass::MODE_LONGDATA_RANGE_LOWPOWER[];
constexpr byte DW1000FRClass::MODE_SHORTDATA_FAST_LOWPOWER[];
constexpr byte DW1000FRClass::MODE_LONGDATA_FAST_LOWPOWER[];
constexpr byte DW1000FRClass::MODE_SHORTDATA_FAST_ACCURACY[];
constexpr byte DW1000FRClass::MODE_LONGDATA_FAST_ACCURACY[];
constexpr byte DW1000FRClass::MODE_LONGDATA_RANGE_ACCURACY[];
/*
...
constexpr byte DW1000FRClass::BIAS_500_16[];
constexpr byte DW1000FRClass::BIAS_500_64[];
constexpr byte DW1000FRClass::BIAS_900_16[];
constexpr byte DW1000FRClass::BIAS_900_64[];
/*
...
*/
// SPI settings
const SPISettings DW1000FRClass::_fastSPI = SPISettings(20000000L, MSBFIRST, SPI_MODE0);
const SPISettings DW1000FRClass::_slowSPI = SPISettings(3000000L, MSBFIRST, SPI_MODE0);
const SPISettings* DW1000FRClass::_currentSPI = &_fastSPI;

/* ###########################################################################
 * #### Init and end #######################################################
 * ######################################################################### */

void DW1000FRClass::end() {
	SPI.end();
}

void DW1000FRClass::select(uint8_t ss) {
	reselect(ss);
...
void DW1000FRClass::reselect(uint8_t ss) {
	_ss = ss;
	pinMode(_ss, OUTPUT);
	digitalWrite(_ss, HIGH);
}

void DW1000FRClass::begin(uint8_t irq, uint8_t rst) {
	...
	attachInterrupt(digitalPinToInterrupt(_irq), DW1000FRClass::handleInterrupt, RISING); // todo interrupt for ESP8266
}

void DW1000FRClass::manageLDE() {
...
void DW1000FRClass::enableClock(byte clock) {
	byte pmscctrl0[LEN_PMSC_CTRL0];
	memset(pmscctrl0, 0, LEN_PMSC_CTRL0);
	readBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, LEN_PMSC_CTRL0);
	if(clock == AUTO_CLOCK) {
...
void DW1000FRClass::enableDebounceClock() {
	byte pmscctrl0[LEN_PMSC_CTRL0];
	memset(pmscctrl0, 0, LEN_PMSC_CTRL0);
	readBytes(PMSC, PMSC_CTRL0_SUB, pmscctrl0, LEN_PMSC_CTRL0);
...
void DW1000FRClass::enableLedBlinking() {
...
void DW1000FRClass::setGPIOMode(uint8_t msgp, uint8_t mode) {
...
void DW1000FRClass::reset() {
	if(_rst == 0xff) {
		softReset();
	} else {
		// DW1000FR data sheet v2.08 §5.6.1 page 20, the RSTn pin should not be driven high but left floating.
		pinMode(_rst, OUTPUT);
		digitalWrite(_rst, LOW);
		delay(2);  // DW1000FR data sheet v2.08 §5.6.1 page 20: nominal 50ns, to be safe take more time
		pinMode(_rst, INPUT);
...
void DW1000FRClass::enableMode(const byte mode[]) {
	setDataRate(mode[0]);
...
void DW1000FRClass::handleInterrupt() {
...
void DW1000FRClass::getPrintableDeviceIdentifier(char msgBuffer[]) {
...
void DW1000FRClass::getPrintableExtendedUniqueIdentifier(char msgBuffer[]) {
	byte data[LEN_EUI];
...
void DW1000FRClass::getPrintableNetworkIdAndShortAddress(char msgBuffer[]) {
...
void DW1000FRClass::getPrintableDeviceMode(char msgBuffer[]) {
...
void DW1000FRClass::readSystemConfigurationRegister() {
	readBytes(SYS_CFG, NO_SUB, _syscfg, LEN_SYS_CFG);
}

void DW1000FRClass::writeSystemConfigurationRegister() {
	writeBytes(SYS_CFG, NO_SUB, _syscfg, LEN_SYS_CFG);
}

void DW1000FRClass::readSystemEventStatusRegister() {
	readBytes(SYS_STATUS, NO_SUB, _sysstatus, LEN_SYS_STATUS);
}

void DW1000FRClass::readNetworkIdAndDeviceAddress() {
	readBytes(PANADR, NO_SUB, _networkAndAddress, LEN_PANADR);
}

void DW1000FRClass::writeNetworkIdAndDeviceAddress() {
	writeBytes(PANADR, NO_SUB, _networkAndAddress, LEN_PANADR);
}

void DW1000FRClass::readSystemEventMaskRegister() {
	readBytes(SYS_MASK, NO_SUB, _sysmask, LEN_SYS_MASK);
}

void DW1000FRClass::writeSystemEventMaskRegister() {
	writeBytes(SYS_MASK, NO_SUB, _sysmask, LEN_SYS_MASK);
}

void DW1000FRClass::readChannelControlRegister() {
	readBytes(CHAN_CTRL, NO_SUB, _chanctrl, LEN_CHAN_CTRL);
}

void DW1000FRClass::writeChannelControlRegister() {
	writeBytes(CHAN_CTRL, NO_SUB, _chanctrl, LEN_CHAN_CTRL);
}

void DW1000FRClass::readTransmitFrameControlRegister() {
	readBytes(TX_FCTRL, NO_SUB, _txfctrl, LEN_TX_FCTRL);
}

void DW1000FRClass::writeTransmitFrameControlRegister() {
	writeBytes(TX_FCTRL, NO_SUB, _txfctrl, LEN_TX_FCTRL);
}

/* ###########################################################################
 * #### DW1000FR operation functions ###########################################
 * ######################################################################### */

void DW1000FRClass::setNetworkId(uint16_t val) {
	_networkAndAddress[2] = (byte)(val & 0xFF);
	_networkAndAddress[3] = (byte)((val >> 8) & 0xFF);
}

void DW1000FRClass::setDeviceAddress(uint16_t val) {
	_networkAndAddress[0] = (byte)(val & 0xFF);
	_networkAndAddress[1] = (byte)((val >> 8) & 0xFF);
}

uint8_t DW1000FRClass::nibbleFromChar(char c) {
	if(c >= '0' && c <= '9') {
		return c-'0';
	}
	if(c >= 'a' && c <= 'f') {
...
void DW1000FRClass::convertToByte(char string[], byte* bytes) {
...
void DW1000FRClass::getTempAndVbat(float& temp, float& vbat) {
...
void DW1000FRClass::setEUI(char eui[]) {
	byte eui_byte[LEN_EUI];
	convertToByte(eui, eui_byte);
	setEUI(eui_byte);
}

void DW1000FRClass::setEUI(byte eui[]) {
...
void DW1000FRClass::setFrameFilter(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFEN_BIT, val);
}

void DW1000FRClass::setFrameFilterBehaveCoordinator(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFBC_BIT, val);
}

void DW1000FRClass::setFrameFilterAllowBeacon(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAB_BIT, val);
}

void DW1000FRClass::setFrameFilterAllowData(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAD_BIT, val);
}

void DW1000FRClass::setFrameFilterAllowAcknowledgement(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAA_BIT, val);
}

void DW1000FRClass::setFrameFilterAllowMAC(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAM_BIT, val);
}

void DW1000FRClass::setFrameFilterAllowReserved(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, FFAR_BIT, val);
}


void DW1000FRClass::setDoubleBuffering(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, DIS_DRXB_BIT, !val);
}

void DW1000FRClass::setInterruptPolarity(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, HIRQ_POL_BIT, val);
}

void DW1000FRClass::setReceiverAutoReenable(boolean val) {
	setBit(_syscfg, LEN_SYS_CFG, RXAUTR_BIT, val);
}

void DW1000FRClass::interruptOnSent(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, TXFRS_BIT, val);
}

void DW1000FRClass::interruptOnReceived(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, RXDFR_BIT, val);
	setBit(_sysmask, LEN_SYS_MASK, RXFCG_BIT, val);
}

void DW1000FRClass::interruptOnReceiveFailed(boolean val) {
...
void DW1000FRClass::interruptOnReceiveTimeout(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, RXRFTO_BIT, val);
}

void DW1000FRClass::interruptOnReceiveTimestampAvailable(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, LDEDONE_BIT, val);
}

void DW1000FRClass::interruptOnAutomaticAcknowledgeTrigger(boolean val) {
	setBit(_sysmask, LEN_SYS_MASK, AAT_BIT, val);
}

void DW1000FRClass::clearInterrupts() {
	memset(_sysmask, 0, LEN_SYS_MASK);
}

void DW1000FRClass::idle() {
...
void DW1000FRClass::startTransmit() {
...
void DW1000FRClass::newConfiguration() {
	idle();
...
void DW1000FRClass::commitConfiguration() {
...
void DW1000FRClass::waitForResponse(boolean val) {
	setBit(_sysctrl, LEN_SYS_CTRL, WAIT4RESP_BIT, val);
}

void DW1000FRClass::suppressFrameCheck(boolean val) {
	_frameCheck = !val;
}

void DW1000FRClass::useSmartPower(boolean smartPower) {
	_smartPower = smartPower;
	setBit(_syscfg, LEN_SYS_CFG, DIS_STXP_BIT, !smartPower);
}

DW1000Time DW1000FRClass::setDelay(const DW1000Time& delay) {
...
void DW1000FRClass::setDataRate(byte rate) {
	rate &= 0x03;
	_txfctrl[1] &= 0x83;
...
void DW1000FRClass::setPulseFrequency(byte freq) {
	freq &= 0x03;
	_txfctrl[2] &= 0xFC;
...
byte DW1000FRClass::getPulseFrequency() {
	return _pulseFrequency;
}

void DW1000FRClass::setPreambleLength(byte prealen) {
	prealen &= 0x0F;
	_txfctrl[2] &= 0xC3;
...
void DW1000FRClass::useExtendedFrameLength(boolean val) {
	_extendedFrameLength = (val ? FRAME_LENGTH_EXTENDED : FRAME_LENGTH_NORMAL);
	_syscfg[2] &= 0xFC;
	_syscfg[2] |= _extendedFrameLength;
}

void DW1000FRClass::receivePermanently(boolean val) {
...
void DW1000FRClass::setChannel(byte channel) {
	channel &= 0xF;
	_chanctrl[0] = ((channel | (channel << 4)) & 0xFF);
	_channel = channel;
}

void DW1000FRClass::setPreambleCode(byte preacode) {
	preacode &= 0x1F;
	_chanctrl[2] &= 0x3F;
...
void DW1000FRClass::setDefaults() {
	if(_deviceMode == TX_MODE) {
...
void DW1000FRClass::setData(byte data[], uint16_t n) {
	if(_frameCheck) {
...
void DW1000FRClass::setData(const String& data) {
...
// TODO reorder
uint16_t DW1000FRClass::getDataLength() {
	uint16_t len = 0;
	if(_deviceMode == TX_MODE) {
...
void DW1000FRClass::getData(byte data[], uint16_t n) {
	if(n <= 0) {
		return;
	}
...
void DW1000FRClass::getData(String& data) {
	uint16_t i;
...
void DW1000FRClass::correctTimestamp(DW1000Time& timestamp) {
...
boolean DW1000FRClass::isReceiveFailed() {
...
boolean DW1000FRClass::isReceiveTimeout() {
	return getBit(_sysstatus, LEN_SYS_STATUS, RXRFTO_BIT);
}

boolean DW1000FRClass::isClockProblem() {
...
void DW1000FRClass::clearTransmitStatus() {
...
float DW1000FRClass::getReceiveQuality() {
...
float DW1000FRClass::getFirstPathPower() {
...
float DW1000FRClass::getReceivePower() {
 ...
 */
void DW1000FRClass::setBit(byte data[], uint16_t n, uint16_t bit, boolean val) {
	uint16_t idx;
	uint8_t shift;
 ...
 */
boolean DW1000FRClass::getBit(byte data[], uint16_t n, uint16_t bit) {
	uint16_t idx;
	uint8_t  shift;
...
void DW1000FRClass::writeValueToBytes(byte data[], int32_t val, uint16_t n) {
	uint16_t i;
	for(i = 0; i < n; i++) {
 ...
 */
// TODO incomplete doc
void DW1000FRClass::readBytes(byte cmd, uint16_t offset, byte data[], uint16_t n) {
	byte header[3];
	uint8_t headerLen = 1;
	uint16_t i = 0;
...
void DW1000FRClass::readBytesOTP(uint16_t address, byte data[]) {
...
void DW1000FRClass::writeByte(byte cmd, uint16_t offset, byte data) {
...
void DW1000FRClass::writeBytes(byte cmd, uint16_t offset, byte data[], uint16_t data_size) {
	byte header[3];
	uint8_t  headerLen = 1;
	uint16_t  i = 0;
...
void DW1000FRClass::getPrettyBytes(byte data[], char msgBuffer[], uint16_t n) {
	uint16_t i, j, b;
...
void DW1000FRClass::getPrettyBytes(byte cmd, uint16_t offset, char msgBuffer[], uint16_t n) {
	uint16_t i, j, b;
